...
protected:
    template <class BasicType>
    static constexpr BasicType To(const std::span<std::byte> bytes)
        requires( std::is_arithmetic_v<BasicType> || std::is_enum_v<BasicType> )
    {
        auto array{ std::bit_cast< std::array<std::byte, sizeof(BasicType)> >(bytes.data()]) };
        BasicType result; // We do this for memory alignment of the variable type

        std::memcpy( &result, &array[0], sizeof(BasicType) );
        return result;
    }
};
